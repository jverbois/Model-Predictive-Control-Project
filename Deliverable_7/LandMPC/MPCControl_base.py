...
class MPCControl_base:
    ...
    def _setup_controller(self) -> None:
        ...
        self.w_par = cp.Parameter(self.nd, name="w")
        ...
        E = self.min_robust_invariant_set(W)
    ...
    def get_u(
        ...
        if np.any(VZ == self.x_ids):
            if self.w_hat is None:
                self.w_hat = np.zeros(self.nd)
            self.w_par.value = self.w_hat
        else:
            self.w_par.value = np.zeros(self.nd)
        ...
        if np.any(VZ == self.x_ids):
            ...
            self.w_hat += innovation[VZ == self.x_ids]
            self.x_hat_next = (
                self.A @ x_traj[:, 0]
                + self.B @ (u_traj[:, 0] - self.us)
                + self.Bd @ self.w_hat
            )
        # YOUR CODE HERE
        #################################################
    ...
    def max_invariant_set(self, X: Polyhedron, max_iter=100) -> Polyhedron:
        """
