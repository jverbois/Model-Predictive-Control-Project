...
class Rocket(RocketBase):
    ...
    J: np.ndarray = field(
        default_factory=lambda: np.diag([0.0644, 0.0644, 0.0128])
    )  # inertia matrix [kg·m²]

    # Geometry/actuation parameters
    r_F: np.ndarray = field(
        default_factory=lambda: np.array([0.0, 0.0, -0.215])
    )  # thrust line offset along z [m]
    thrust_coeff: np.ndarray = field(
        default_factory=lambda: np.array([0.0, 0.0, 0.0])
    )  # [a2, a1, a0]
    torque_coeff: np.ndarray = field(
        default_factory=lambda: np.array([0.0, 0.0, 0.0])
    )  # [b2, b1, b0]
    ...
    def f(self, x: np.ndarray, u: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Twb = self.eul2mat(r)
        v_dot = Twb @ (b_F / float(self.mass - self.fuel_consumption)) - np.array(
            [0, 0, self.g]
        )
        p_dot = v
    ...
    def getForceAndMomentFromThrust(
        self, u: np.ndarray
        ...
        b_eF = np.array(
            [np.sin(u[1]), -np.sin(u[0]) * np.cos(u[1]), np.cos(u[0]) * np.cos(u[1])]
        )
        b_F = thrust * b_eF
    ...
    def f_symbolic(self, x: ca.SX, u: ca.SX, mass: ca.SX = None) -> ca.SX:
        ...
        E_inv = (1 / ca.cos(bet)) * ca.vertcat(
            ...
            ca.horzcat(
                -ca.cos(gam) * ca.sin(bet), ca.sin(gam) * ca.sin(bet), ca.cos(bet)
            ),
        )
        r_dot = E_inv @ w
    ...
    def getForceAndMomentFromThrust_symbolic(self, u: ca.SX) -> Tuple[ca.SX, ca.SX]:
        """Compute thrust force and moment in body frame."""
        thrust = self.g * (
            ca.DM(self.thrust_coeff.reshape(1, -1)) @ ca.vertcat(u[2] ** 2, u[2], 1.0)
        )
        torque = self.J[2, 2] * (
            ca.DM(self.torque_coeff.reshape(1, -1)) @ ca.vertcat(u[3] ** 2, u[3], 1.0)
        )

        b_eF = ca.vertcat(
            ca.sin(u[1]), -ca.sin(u[0]) * ca.cos(u[1]), ca.cos(u[0]) * ca.cos(u[1])
        )
        b_F = thrust * b_eF
    ...
    def decompose(A: np.ndarray, B: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        xids_x = np.array([1, 4, 6, 9])
        uids_x = np.array([1])
        xids_xi, xids_xj = np.meshgrid(xids_x, xids_x)
        uids_xi, uids_xj = np.meshgrid(xids_x, uids_x)
        Ax = A[xids_xi, xids_xj].T
        Bx = B[uids_xi, uids_xj].T

        """Thrust vector angle dR to position y"""
        xids_y = np.array([0, 3, 7, 10])
        uids_y = np.array([0])
        xids_yi, xids_yj = np.meshgrid(xids_y, xids_y)
        uids_yi, uids_yj = np.meshgrid(xids_y, uids_y)
        Ay = A[xids_yi, xids_yj].T
        By = B[uids_yi, uids_yj].T

        """Average throttle Pavg to height z"""
        xids_z = np.array([8, 11])
        uids_z = np.array([2])
        xids_zi, xids_zj = np.meshgrid(xids_z, xids_z)
        uids_zi, uids_zj = np.meshgrid(xids_z, uids_z)
        Az = A[xids_zi, xids_zj].T
        Bz = B[uids_zi, uids_zj].T

        """Differential throttle to roll angle psi"""
        xids_p = np.array([2, 5])
        uids_p = np.array([3])
        xids_pi, xids_pj = np.meshgrid(xids_p, xids_p)
        uids_pi, uids_pj = np.meshgrid(xids_p, uids_p)
        Ap = A[xids_pi, xids_pj].T
        Bp = B[uids_pi, uids_pj].T
    ...
    def simulate(
        self, x0: np.ndarray, Tf: float, U: np.ndarray, method: str = "nonlinear"
        ...
        if method == "linear":
            ...
            for k in range(N):
                # Euler integration
                X[:, k + 1] = Rocket.linear_integrate_step(
                    self.A_ss, self.xs, self.B_ss, self.us, X[:, k], U[:, k], self.Ts
                )
        else:
            ...
            for k in range(N):
                ...
                X[:, k + 1] = Rocket.integrate_step(
                    current_rocket.f, X[:, k], U[:, k], self.Ts
                )
    ...
    def simulate_step(
        ...
        if ~constraint_check(x0, u0):
            print("Warning: State or input constraints violated")
        ...
        if method == "linear":
            ...
            if not hasattr(self, "A_ss") or not hasattr(self, "B_ss"):
                self.linearize(self.xs, self.us)
            return Rocket.linear_integrate_step(
                self.A_ss, self.xs, self.B_ss, self.us, x0, u0, Ts
            )
        else:
            if method == "nonlinear_real":
                current_rocket = perturb_rocket(self)
    ...
    def simulate_control(
        ...
        x_cl = np.zeros((12, N_cl + 1), dtype=float)
        ...
        for k in range(N_cl):
            ...
            u_cl[:, k], x_ol[..., k], u_ol[..., k], t_ol[..., k] = mpc.get_u(
                t_cl[k], x_cl[:, k], x_target=x_target[:, k], u_target=u_target
            )
            u_cl[:, k] = current_rocket.fuel_dynamics(u_cl[:, k], self.Ts)
            x_cl[:, k + 1] = current_rocket.simulate_step(
                x_cl[:, k], self.Ts, u_cl[:, k], method=method
            )
    ...
    def simulate_land(
        self, mpc, sim_time: float, H: float, x0: np.ndarray, method: str = "nonlinear"
    ):
        N_cl = int(sim_time / self.Ts)
        N_ol = int(H / self.Ts)
        ...
        if method == "nonlinear_real":
            ...
            for k in range(N_cl):
                ...
                x_cl[:, k + 1] = realistic_rocket.simulate_step(
                    x_cl[:, k], self.Ts, u_cl[:, k], method="nonlinear"
                )
                t_cl[k + 1] = t_cl[k] + self.Ts
        else:
            # Closed-loop simulation
        ...
        match flag_noise:
            ...
            case 3:
                ...
                noise["w"] = np.concatenate(
                    [np.full(t1, w_min), np.full(num_step - t1, w_max)]
                )
            case _:
    ...
    def simulate_subsystem(self, mpc, sim_time: float, x0: np.ndarray, w_type: str):
        ...
        for k in range(N_cl):
            ...
            x_cl[mpc.x_ids, k + 1] = self.simulate_step_dt(
                x_cl[mpc.x_ids, k], u_cl[mpc.u_ids, k], w_traj_cl[k], mpc
            )
            t_cl[k + 1] = t_cl[k] + Ts
...
def perturb_rocket(rocket, seed: int | None = 1):
    ...
    dJ = (0.4 * r.J) @ np.diag(
        rand_plusminus_one(3)
    )  # mirrors MATLAB: (0.4*J)*diag(rand)
    r.mass = r.mass + dMass
    r.J = r.J + dJ
...
def constraint_check(x: np.ndarray, u: np.ndarray) -> bool:
    ...
    LBU = np.array(
        [-np.deg2rad(15), -np.deg2rad(15), 40.0, -20.0]
    )  # [dR, dP, Pavg, Pdiff]
    UBU = np.array(
        [np.deg2rad(15), np.deg2rad(15), 80.0, 20.0]
    )  # [dR, dP, Pavg, Pdiff]
    ...
    return (
        ...
        and np.all(x >= LBX - 1e-5)
        and np.all(x <= UBX + 1e-5)
    )
...
class LTISys:
    ...
    def _parse_subsystem(
        self, idx: np.ndarray, idu: np.ndarray, idy: np.ndarray, name: str
    ):
    ...
    def _discretize(self, Ts: float):
        ...
        A_discrete, B_discrete, C_discrete, _, _ = cont2discrete(
            system=(self.A, self.B, self.C, self.D), dt=Ts
        )
